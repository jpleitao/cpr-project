...
"""
Implementation of the Dynamic Time Warping Barycenter Average to compute the Time Series' centroid, for a given cluster
Implementation highly based on the implementation available on:
    https://github.com/brandonckelly/bck_stats/blob/master/bck_stats/dba.py
"""
...
__author__ = 'brandonkelly', 'Joaquim Leitão'
__copyright__ = 'Copyright (c) 2017 Joaquim Leitão'
__email__ = 'jocaleitao93@gmail.com'


def ts_to_dba_list(series_list):
    ...
    for i in range(1, num_series):
        result = np.column_stack((result, series_list[i]))
    return result
...
def dynamic_time_warping(tseries1, tseries2):
    ...
    """
    result = tseries1.shape
    if len(result) == 1:
        tseries1 = np.array([tseries1]).transpose()
    ...
    def compute_average(self, tseries, nstarts=1, initial_value=None, dba_length=None):
        ...
        for i in range(nstarts):
            ...
            if self.wgss < best_wgss:
                ...
                if self.verbose:
                    print('New best estimate found for random start ' + str(i))
                best_wgss = self.wgss
...
if __name__ == "__main__":
    ...
    for i in range(nseries):
        ...
        tseries[:, 2] = np.sin(t / period2[i] + phase2[i]) + noise_amplitude * np.random.standard_normal(ntime)
